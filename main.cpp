...
int main(){
    ...
    */
    ...
    while (window.isOpen()){
        ...
        rowdestroy();		// function call for destroying the completed rows.
         
        if(rotate)		// function call for rotating the block when up key is pressed
        {
      		rotation();
      		rotate=false;		// resetting the value of rotate to false after one rotation.
        }
       
        if(game_end()==true)		// function call for closing the window when any column of the top row is filled.
        {
      		window.close();
        }
